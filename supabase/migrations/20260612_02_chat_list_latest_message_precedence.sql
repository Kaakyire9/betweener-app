...
begin
  ...
  where summary.owner_user_id = p_owner_user_id
    ...
    and (
      summary.last_activity_kind = 'reaction'
      or summary.last_activity_message_id is distinct from summary.last_message_id
    );
end;
$$;
...
begin
  ...
  set last_message_reaction_emoji = v_reaction_emoji,
      ...
      last_activity_kind = case
        when summary.last_activity_kind = 'reaction' then null
        else summary.last_activity_kind
      end,
      last_activity_message_id = case
        when summary.last_activity_kind = 'reaction' then null
        else summary.last_activity_message_id
      end,
      last_activity_preview = case
        when summary.last_activity_kind = 'reaction' then null
        else summary.last_activity_preview
      end,
      last_activity_at = case
        when summary.last_activity_kind = 'reaction' then null
        else summary.last_activity_at
      end,
      updated_at = timezone('utc'::text, now())
