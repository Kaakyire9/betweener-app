...
drop function if exists public.rpc_prepare_view_once_attachment(uuid);
drop function if exists public.rpc_complete_view_once_attachment(uuid);

create or replace function public.rpc_prepare_view_once_attachment(p_message_id uuid)
returns table (
  attachment_id uuid,
  bucket_id text,
  storage_path text,
...
begin
  ...
  if exists (
    select 1
    from public.message_views view_row
...
create or replace function public.rpc_complete_view_once_attachment(p_message_id uuid)
returns table (
  attachment_id uuid,
...
declare
  v_message public.messages%rowtype;
  v_attachment public.message_attachments%rowtype;
  v_consumed_at timestamptz := timezone('utc', now());
begin
  if auth.uid() is null then
    raise exception using errcode = '42501', message = 'authentication_required';
  end if;
  ...
  set delivered_at = coalesce(public.messages.delivered_at, v_consumed_at),
      read_at = coalesce(public.messages.read_at, v_consumed_at),
      is_read = true
  where public.messages.id = p_message_id;

  update public.message_attachments
  set expires_at = v_consumed_at + interval '5 minutes',
      updated_at = v_consumed_at
  ...
  insert into public.chat_attachment_cleanup_queue (
    ...
    v_attachment.storage_path,
    'view_once_consumed',
    v_consumed_at + interval '5 minutes'
  )
  ...
  return query
  select v_attachment.id, v_consumed_at;
end;
$$;

revoke all on function public.rpc_prepare_view_once_attachment(uuid) from public, anon;
grant execute on function public.rpc_prepare_view_once_attachment(uuid) to authenticated;

revoke all on function public.rpc_complete_view_once_attachment(uuid) from public, anon;
grant execute on function public.rpc_complete_view_once_attachment(uuid) to authenticated;
