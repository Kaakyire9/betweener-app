-- Enrich profile interest with person-based metrics and stronger signal story inputs.
...
begin
  ...
  current_saves as (
    select save_row.viewer_profile_id
    from public.profile_saves save_row
    where save_row.target_profile_id = v_profile_id
      and save_row.created_at > timezone('utc'::text, now()) - v_window
  ),
  signal_people as (
    select scoped.viewer_profile_id from scoped
    union
    select current_saves.viewer_profile_id from current_saves
  ),
  grouped as (
    select
  ...
  ranked_people as (
    select
      ...
      actor.avatar_url,
      case
        when enriched.profile_save_count > 0 then 'profile_saved'
        when enriched.intent_open_count > 0 then 'intent_opened'
        when enriched.intro_watch_count > 0 then 'intro_completed'
    ...
    order by enriched.interest_score desc, enriched.last_signal_at desc
    limit case
