-- Boosts V2:
-- 1) keep the existing boost entrypoint compatible
-- 2) add premium-only precision controls for Gold
-- 3) expose recommendation and analytics RPCs for the app

alter table public.profile_boosts
  add column if not exists boost_type text not null default 'manual',
  add column if not exists audience_mode text not null default 'for_you',
  add column if not exists focus_mode text not null default 'profile',
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'profile_boosts_boost_type_check'
      and conrelid = 'public.profile_boosts'::regclass
  ) then
    alter table public.profile_boosts
      add constraint profile_boosts_boost_type_check
      check (boost_type in ('manual', 'smart'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'profile_boosts_audience_mode_check'
      and conrelid = 'public.profile_boosts'::regclass
  ) then
    alter table public.profile_boosts
      add constraint profile_boosts_audience_mode_check
      check (audience_mode in ('for_you', 'nearby', 'active_now', 'intent_match', 'second_look'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'profile_boosts_focus_mode_check'
      and conrelid = 'public.profile_boosts'::regclass
  ) then
    alter table public.profile_boosts
      add constraint profile_boosts_focus_mode_check
      check (focus_mode in ('profile', 'intro', 'intent'));
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'profile_boosts_status_check'
      and conrelid = 'public.profile_boosts'::regclass
  ) then
    alter table public.profile_boosts
      add constraint profile_boosts_status_check
      check (status in ('active', 'completed'));
  end if;
end $$;

create index if not exists idx_profile_boosts_active_window
  on public.profile_boosts (user_id, starts_at desc, ends_at desc);
...
begin
  ...
  return jsonb_strip_nulls(jsonb_build_object(
    ...
    'active_boost_id', active_boost.id,
    'active_boost_type', active_boost.boost_type,
    'active_boost_audience_mode', active_boost.audience_mode,
    'active_boost_focus_mode', active_boost.focus_mode
  ));
end;
$$;
...
begin
  ...
  with recent_events as (
    ...
    where event_row.target_profile_id = viewer_profile_id
...
begin
  ...
  where match_row.status = 'ACCEPTED'
    ...
    and (
      match_row.user1_id = viewer_profile_id
      or match_row.user2_id = viewer_profile_id
    );
...
as $$
begin
  return public.rpc_create_profile_boost_v2(
    'manual',
    'for_you',
    'profile',
    '{}'::jsonb
...
revoke all on function public.rpc_get_profile_boost_recommendations() from public;
revoke all on function public.rpc_get_my_recent_boost_analytics() from public;
revoke all on function public.rpc_create_profile_boost_v2(text, text, text, jsonb) from public;
revoke all on function public.rpc_create_profile_boost() from public;

grant execute on function public.rpc_get_profile_boost_recommendations() to authenticated;
grant execute on function public.rpc_get_my_recent_boost_analytics() to authenticated;
grant execute on function public.rpc_create_profile_boost_v2(text, text, text, jsonb) to authenticated;
