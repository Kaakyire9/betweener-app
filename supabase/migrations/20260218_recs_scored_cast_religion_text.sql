...
--   DETAIL: Returned type religion does not match expected type text in column 13.
--
-- In our schema, `profiles.religion` is an enum type `religion`, while our RPCs
-- declare `religion text` in RETURNS TABLE for client compatibility.
-- Postgres does not implicitly coerce enum -> text for RETURN QUERY in RETURNS TABLE
-- functions, so we must cast.
