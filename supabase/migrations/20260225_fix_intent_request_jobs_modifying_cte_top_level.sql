-- Fix rpc_process_intent_request_jobs:
-- Keep data-modifying CTEs (INSERT) at the top level.
--
-- Error addressed:
--   SQLSTATE 0A000: WITH clause containing a data-modifying statement must be at the top level
--
-- Root cause:
--   A previous version nested an INSERT CTE inside a subquery, which Postgres forbids.
...
BEGIN
  ...
  --
  -- Single top-level statement so the INSERT CTE is valid and the to_send scope is preserved.
  WITH candidates AS (
    SELECT
      ir.id AS request_id,
