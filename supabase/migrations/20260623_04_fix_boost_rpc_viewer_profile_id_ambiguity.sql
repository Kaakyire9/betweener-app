-- Fix PL/pgSQL variable/column ambiguity in boost RPCs. `vibes_events` contains a
-- `viewer_profile_id` column, so bare references inside these functions can resolve
-- ambiguously once the RPC queries that table.
