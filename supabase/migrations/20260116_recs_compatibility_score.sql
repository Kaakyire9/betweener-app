-- Migration: per-viewer compatibility scoring for recommendations (ai_score = 0..100)
-- Adds scored variants of existing recommendation RPCs so the app can show a true "% Match" per viewer/target.
...
begin
  ...
  select latitude, longitude, user_id, min_age_interest, max_age_interest, region
    into my_lat, my_lon, my_user_id, my_min_age, my_max_age, my_region
...
begin
  ...
  select user_id, min_age_interest, max_age_interest
    into my_user_id, my_min_age, my_max_age
  from public.profiles
  where id = p_user_id
    and user_id = auth.uid()
  limit 1;
