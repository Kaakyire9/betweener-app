alter table public.vibes_events
  drop constraint if exists vibes_events_type_valid;

alter table public.vibes_events
  add constraint vibes_events_type_valid check (event_type in (
    'card_seen',
    'profile_opened',
    'full_profile_opened',
    'intro_played',
...
create table if not exists public.profile_saves (
  ...
  constraint profile_saves_no_self check (viewer_profile_id <> target_profile_id)
);

create index if not exists profile_saves_target_created_idx
  on public.profile_saves (target_profile_id, created_at desc);

alter table public.profile_saves enable row level security;
revoke all on public.profile_saves from anon, authenticated;
...
create or replace function public.rpc_set_profile_saved(
  p_viewer_profile_id uuid,
  p_target_profile_id uuid,
  p_saved boolean
)
...
begin
  ...
  if v_owner_user_id is distinct from auth.uid()
    or p_target_profile_id is null
    or p_target_profile_id = p_viewer_profile_id then
    raise exception 'not_authorized' using errcode = '42501';
  end if;

  if coalesce(p_saved, false) then
    insert into public.profile_saves (viewer_profile_id, target_profile_id)
    values (p_viewer_profile_id, p_target_profile_id)
    on conflict (viewer_profile_id, target_profile_id) do nothing;
    ...
    perform public.rpc_log_vibes_event(
      ...
      null,
      null,
      jsonb_build_object('source', 'profile_view')
    );
    return true;
  end if;

  delete from public.profile_saves save_row
  where save_row.viewer_profile_id = p_viewer_profile_id
    and save_row.target_profile_id = p_target_profile_id;

  return false;
end;
$$;

create or replace function public.rpc_is_profile_saved(
  p_viewer_profile_id uuid,
  p_target_profile_id uuid
)
returns boolean
...
as $$
  select exists (
    ...
    join public.profiles viewer on viewer.id = save_row.viewer_profile_id
    where save_row.viewer_profile_id = p_viewer_profile_id
      and save_row.target_profile_id = p_target_profile_id
...
as $$
  ...
  interest as (
    select
      ...
      least(100, (
        count(*) filter (where event_row.event_type = 'profile_opened')
        + count(*) filter (where event_row.event_type = 'full_profile_opened') * 2
        + count(*) filter (where event_row.event_type in ('intro_played', 'intro_completed')) * 3
        + count(*) filter (where event_row.event_type = 'profile_saved') * 4
        + greatest(
          count(*) filter (where event_row.event_type in ('profile_opened', 'full_profile_opened')) - 1,
          0
        ) * 5
        + count(*) filter (where event_row.event_type = 'intent_opened') * 5
      ))::integer as score
    from public.vibes_events event_row
    ...
    group by event_row.viewer_profile_id
  )
  select
    profile.id,
  ...
  if v_plan in ('SILVER', 'GOLD') then
    ...
    grouped as (
      select
        ...
        least(100, (
          count(*) filter (where scoped.event_type = 'profile_opened')
          + count(*) filter (where scoped.event_type = 'full_profile_opened') * 2
          + count(*) filter (where scoped.event_type in ('intro_played', 'intro_completed')) * 3
          + count(*) filter (where scoped.event_type = 'profile_saved') * 4
          + greatest(count(*) filter (where scoped.event_type in ('profile_opened', 'full_profile_opened')) - 1, 0) * 5
          + count(*) filter (where scoped.event_type = 'intent_opened') * 5
    ...
    select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      ...
      'interest_level', case
        when ranked_people.interest_score >= 16 then 'High Interest'
        when ranked_people.interest_score >= 7 then 'Medium Interest'
        else 'Low Interest'
      end,
...
revoke all on function public.rpc_set_profile_saved(uuid, uuid, boolean) from public;
revoke all on function public.rpc_is_profile_saved(uuid, uuid) from public;
revoke all on function public.rpc_get_profile_card_context(uuid[]) from public;
...
grant execute on function public.rpc_set_profile_saved(uuid, uuid, boolean) to authenticated;
grant execute on function public.rpc_is_profile_saved(uuid, uuid) to authenticated;
grant execute on function public.rpc_get_profile_card_context(uuid[]) to authenticated;
