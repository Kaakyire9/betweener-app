...
begin
  ...
  fallback_candidates as (
    ...
    from base_candidates base_candidate
    cross join high_quality_primary_count primary_count
    where primary_count.total < 6
      and base_candidate.within_viewer_age_pref
      and not base_candidate.within_target_age_pref
      and base_candidate.viewer_binary_gender_match
      and base_candidate.target_binary_gender_match
  ),
  ...
  ranked as (
    select
      ...
      (
        ...
        - case when scored.candidate_tier = 1 then 1.9 else 0 end
