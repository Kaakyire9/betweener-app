-- Keep Active Now RPCs aligned with Nearby/For You so the client can rely on
-- distance_km across all Vibes tabs.
...
begin
  ...
  order by 20 desc, p.online desc, coalesce(p.last_active, cutoff) desc, 21 asc nulls last, p.updated_at desc
  limit 50;
end;
$$;
...
begin
  ...
  return query
    ...
    order by p.online desc, coalesce(p.last_active, cutoff) desc, distance_km asc nulls last, p.updated_at desc
    limit 50;
end;
$$;
...
set current_country_code = case lower(trim(current_country))
  when 'ghana' then 'GH'
  ...
  when 'uae' then 'AE'
  else current_country_code
end
where coalesce(trim(current_country_code), '') = ''
  and coalesce(trim(current_country), '') <> '';
