-- Vibes V5: reciprocal, outcome-aware recommendations with transferable taste learning.
--
-- V5 deliberately builds on the proven V4 candidate generator exposed through
-- get_vibes_recommendations_v3. This keeps all existing safety, privacy,
-- location, freshness, diversity and entitlement behaviour while adding:
--   * reciprocal age / explicit Compass eligibility;
--   * pair-level intention, values and lifestyle compatibility;
--   * time-decayed taste learned from prior actions;
--   * asynchronous taste refreshes and a safe V4 fallback contract.

create table if not exists public.vibes_v5_viewer_feature_weights (
  ...
  primary key (viewer_profile_id, feature_key, feature_value),
  constraint vibes_v5_feature_key_valid check (
    feature_key in (
      'interest', 'intention', 'personality', 'smoking', 'drinking',
      'exercise', 'children', 'love_language', 'intro_media', 'activity'
    )
  ),
  constraint vibes_v5_feature_value_present check (btrim(feature_value) <> ''),
  constraint vibes_v5_feature_weight_bounded check (weight between -1 and 1),
  constraint vibes_v5_feature_evidence_valid check (
    positive_evidence >= 0 and negative_evidence >= 0 and evidence_count >= 0
  )
);

create index if not exists vibes_v5_viewer_feature_weights_lookup_idx
  on public.vibes_v5_viewer_feature_weights (viewer_profile_id, feature_key, weight desc);

create table if not exists public.vibes_v5_taste_refresh_queue (
  viewer_profile_id uuid primary key references public.profiles(id) on delete cascade,
  requested_at timestamptz not null default timezone('utc', now()),
  attempts integer not null default 0,
  last_error text,
  constraint vibes_v5_taste_queue_attempts_valid check (attempts >= 0)
);

alter table public.vibes_v5_viewer_feature_weights enable row level security;
alter table public.vibes_v5_taste_refresh_queue enable row level security;

revoke all on table public.vibes_v5_viewer_feature_weights from public, anon, authenticated;
revoke all on table public.vibes_v5_taste_refresh_queue from public, anon, authenticated;
grant select, insert, update, delete on table public.vibes_v5_viewer_feature_weights to service_role;
grant select, insert, update, delete on table public.vibes_v5_taste_refresh_queue to service_role;

create or replace function public.normalize_vibes_v5_intention(p_value text)
returns text
language sql
immutable
parallel safe
...
as $$
  select case
    when nullif(lower(btrim(coalesce(p_value, ''))), '') is null then null
    when lower(p_value) ~ '(marriage|married|wife|husband|long.?term|serious|life partner|committed)'
      then 'committed'
    when lower(p_value) ~ '(casual|short.?term|fun|no strings|hookup)'
      then 'casual'
    when lower(p_value) ~ '(friend|community|network)'
      then 'friendship'
    when lower(p_value) ~ '(open|see what|chemistry|explor)'
      then 'open'
    else lower(btrim(p_value))
  end;
$$;

revoke all on function public.normalize_vibes_v5_intention(text) from public;
grant execute on function public.normalize_vibes_v5_intention(text) to authenticated, service_role;
...
begin
  ...
  delete from public.vibes_v5_viewer_feature_weights weights
  ...
  with event_evidence as (
    select
      ...
      case event_row.event_type
        when 'intent_sent' then 9.0
        when 'signal_sent' then 8.0
        when 'like' then 7.0
        when 'profile_saved' then 5.5
        when 'intro_completed' then 3.8
        when 'full_profile_opened' then 2.8
        when 'intro_played' then 2.0
        when 'profile_opened' then 1.4
        when 'pass' then -6.5
        when 'undo' then 1.0
        else case
          when coalesce(event_row.dwell_ms, 0) >= 12000 then 2.2
          when coalesce(event_row.dwell_ms, 0) >= 4000 then 1.0
          else 0
        end
      end
      * exp(
          -greatest(
            extract(epoch from (timezone('utc', now()) - event_row.created_at)) / 86400.0,
            0
          ) / 75.0
        ) as evidence
  ...
  pair_evidence as (
    select
      event_evidence.target_profile_id,
      greatest(-12.0, least(16.0, sum(event_evidence.evidence))) as evidence,
      count(*)::integer as evidence_count
    from event_evidence
    where event_evidence.evidence <> 0
    group by event_evidence.target_profile_id
  ),
  target_features as (
    select distinct
      pair_evidence.target_profile_id,
      pair_evidence.evidence,
      pair_evidence.evidence_count,
      feature.feature_key,
      feature.feature_value
    from pair_evidence
    join public.profiles target
      on target.id = pair_evidence.target_profile_id
    ...
    cross join lateral (
      ...
      union all select 'activity', case when target.last_active >= timezone('utc', now()) - interval '3 days' then 'recent' end
      union all
  ...
  aggregated as (
    select
      ...
      sum(target_features.evidence) as net_evidence,
      sum(greatest(target_features.evidence, 0)) as positive_evidence,
      sum(abs(least(target_features.evidence, 0))) as negative_evidence,
      sum(target_features.evidence_count)::integer as evidence_count
    from target_features
    group by target_features.feature_key, target_features.feature_value
  ),
  upserted as (
    ...
    select
      ...
      greatest(
        ...
        least(
          1.0,
          aggregated.net_evidence
            / greatest(18.0, aggregated.positive_evidence + aggregated.negative_evidence)
        )
      ),
      aggregated.positive_evidence,
      aggregated.negative_evidence,
      aggregated.evidence_count,
      timezone('utc', now())
    from aggregated
    where aggregated.evidence_count > 0
      and abs(aggregated.net_evidence) >= 0.35
  ...
  delete from public.vibes_v5_taste_refresh_queue queue
...
create or replace function public.trg_enqueue_vibes_v5_taste_refresh()
returns trigger
language plpgsql
security definer
set search_path = public, pg_catalog
as $$
begin
  if new.viewer_profile_id is not null and new.event_type in (
    'card_seen', 'profile_opened', 'full_profile_opened', 'intro_played',
    'intro_completed', 'profile_saved', 'pass', 'like', 'signal_sent',
    'intent_sent', 'undo'
  ) then
    ...
    values (new.viewer_profile_id, timezone('utc', now()), 0, null)
...
after insert on public.vibes_events
for each row execute function public.trg_enqueue_vibes_v5_taste_refresh();

revoke all on function public.trg_enqueue_vibes_v5_taste_refresh() from public, anon, authenticated;
...
-- Prefer a database-owned worker where pg_cron is available. The guarded block
-- keeps local and restricted environments deployable; those environments can
-- invoke the worker from their existing scheduler instead.
do $$
declare
  v_existing_job_id bigint;
begin
  if to_regprocedure('cron.schedule(text,text,text)') is null then
    raise notice 'pg_cron is unavailable; schedule rpc_process_vibes_v5_taste_jobs externally';
    return;
  end if;

  select jobid into v_existing_job_id
  from cron.job
  where jobname = 'vibes-v5-taste-refresh'
  limit 1;
  ...
  perform cron.schedule(
    'vibes-v5-taste-refresh',
    '*/5 * * * *',
    'select public.rpc_process_vibes_v5_taste_jobs(150);'
  );
exception
  when insufficient_privilege then
    raise notice 'Unable to schedule V5 taste refresh; configure the worker with service-role scheduling';
  when undefined_table or undefined_function then
    raise notice 'pg_cron metadata is unavailable; configure the V5 worker externally';
end;
$$;
        ...
        'public_reasons', (
          ...
          from jsonb_array_elements_text(
            ...
            || to_jsonb(array_remove(array[
              ...
              case when ordered.candidate_min_age is not null and ordered.candidate_max_age is not null then 'reciprocal_fit' end
            ]::text[], null))
          ) reason(value)
        )
...
comment on function public.get_vibes_recommendations_v5(uuid, text, integer, integer) is
  'V5 reciprocal hybrid recommender. Builds on V4 retrieval, adds explicit mutual eligibility, Compass compatibility, time-decayed taste learning, privacy-safe output and controlled exploration.';
