-- A Private Spark is an independent, mutually-consented room. Admission must
-- not depend on the mutable presence state of the source public session: a
-- participant can legitimately leave that transport during the handoff.
...
begin
  ...
  select count(distinct response.user_id)
