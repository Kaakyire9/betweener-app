-- Tune Vibes Recommendation Engine v2:
-- - Do not trust stale profiles.online/is_active for Active Now.
-- - Let old PASS decisions re-enter after a cooldown so thin pools do not feel empty.
-- - Keep enum casts from the previous return-type fix.
...
begin
    ...
    where p.id <> p_user_id
      ...
      and not exists (
        ...
        where s.swiper_id = p_user_id
          ...
          and (
            s.action::text in ('LIKE', 'SUPERLIKE')
            or (
              s.action::text = 'PASS'
  ...
  select
    ...
    r.personality_type,
    (r.last_active is not null and r.last_active >= now() - interval '15 minutes') as is_active,
    (r.last_active is not null and r.last_active >= now() - interval '3 minutes') as online,
    r.last_active,
