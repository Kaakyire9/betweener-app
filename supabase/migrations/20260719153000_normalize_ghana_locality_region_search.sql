-- Accept legacy labels such as "Greater Accra Region" while keeping the
-- canonical Ghana locality catalogue keyed by "Greater Accra".
...
as $$
  with input as (
    select
      ...
      nullif(
        regexp_replace(
          lower(btrim(regexp_replace(coalesce(p_query, ''), '[^a-zA-Z0-9\s-]+', ' ', 'g'))),
          '\s+',
          ' ',
          'g'
        ),
...
grant execute on function public.search_ghana_localities(text, text, integer) to anon, authenticated, service_role;
