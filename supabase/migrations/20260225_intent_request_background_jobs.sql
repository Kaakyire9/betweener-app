-- Background jobs for Intent Requests:
-- - Mark pending requests as expired when expires_at passes (even if the app is closed)
-- - Send a single "closing soon" push reminder to the recipient (rate-limited)
-- - Add a premium "coach" system message to the actor when a request expires
--
-- Scheduling:
--   Run `rpc_process_intent_request_jobs()` periodically (e.g. every 15 minutes) via:
--   - Supabase Scheduled Edge Function (recommended), or
--   - External cron calling the `intent-jobs` edge function.

-- Track push nudges so we never spam the same user about the same request.
CREATE TABLE IF NOT EXISTS public.intent_request_nudges (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  intent_request_id uuid NOT NULL REFERENCES public.intent_requests (id) ON DELETE CASCADE,
  kind text NOT NULL CHECK (kind IN ('recipient_expiring_soon')),
  user_id uuid NOT NULL REFERENCES auth.users (id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  CONSTRAINT intent_request_nudges_unique UNIQUE (intent_request_id, kind, user_id)
);

CREATE INDEX IF NOT EXISTS idx_intent_request_nudges_user_created
  ON public.intent_request_nudges (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_intent_request_nudges_kind_created
  ON public.intent_request_nudges (kind, created_at DESC);

ALTER TABLE public.intent_request_nudges ENABLE ROW LEVEL SECURITY;
-- No policies: only service role/background jobs should read/write this table.
...
BEGIN
  ...
  --
  -- The windowed check lets you run this job frequently without duplicates:
  --   expires_at - now() <= p_remind_before
  --   expires_at - now() >  p_remind_before - p_window
  WITH candidates AS (
    SELECT
      ir.id AS request_id,
  ...
  SELECT count(*) INTO v_reminders_sent FROM to_send;

  -- Fire-and-forget push webhook calls (best-effort).
...
-- Lock down execution: background jobs only.
