-- Cross-Circle Picks for the Circles home. This is a private dating projection,
-- never a community-member aggregation. Existing Circle consent, eligibility,
-- passes, Intent, Match and Chat remain authoritative.
...
begin
  ...
  with viewer as (
    ...
    select
      ...
      candidate.city as candidate_city,
      candidate.current_country as candidate_country,
      candidate.looking_for as candidate_intent,
    ...
    left join lateral (
      ...
      where event_row.viewer_profile_id = viewer.id
        ...
        and event_row.created_at >= timezone('utc', now()) - interval '7 days'
    ) exposure on true
    where public.is_romantically_eligible(viewer.id, candidate.id, 'circle', circle_row.id)
      ...
      -- A home Pick passed through any shared Circle stays suppressed globally
      -- in Circle Picks for 14 days, while retaining the existing pass table.
      and not exists (
  ...
  order by scored.internal_score desc,
