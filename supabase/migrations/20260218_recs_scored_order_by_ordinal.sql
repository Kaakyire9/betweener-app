...
--   PostgREST; error=42702 (ambiguous_column)
-- caused by ORDER BY referencing output-column names in PL/pgSQL RETURNS TABLE functions.
-- In PL/pgSQL, output columns are variables, so `ORDER BY ai_score` / `distance_km`
-- can become ambiguous. Use ordinal positions instead.
