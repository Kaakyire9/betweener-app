-- Align profile-interest counts with active viewer profiles and current save state.
...
begin
  ...
  ranked_people as (
    ...
    order by enriched.interest_score desc, enriched.last_signal_at desc nulls last
    limit case
  ...
  )) order by ranked_people.interest_score desc, ranked_people.last_signal_at desc nulls last), '[]'::jsonb)
    into v_people
  from ranked_people;
