-- Filter recommendations to avoid same-sex cards by default (binary gender only).
-- Current product expectation: MALE profiles see FEMALE profiles, and vice-versa.
-- NON_BINARY/OTHER/NULL falls back to "no gender filter" until we add an explicit preference field.
...
begin
  ...
  select latitude, longitude, user_id, min_age_interest, max_age_interest, region, gender
  ...
  order by ai_score desc, distance_km asc nulls last, p.updated_at desc
  limit p_limit;
end;
$$;
...
begin
  ...
  select user_id, min_age_interest, max_age_interest, gender
  ...
  order by ai_score desc, p.online desc, coalesce(p.last_active, cutoff) desc, p.updated_at desc
  limit 50;
end;
$$;
...
-- Legacy RPCs used as a fallback in some clients. Keep them safe and aligned.
...
begin
  ...
  select latitude, longitude, user_id, gender
    into my_lat, my_lon, my_user_id, my_gender
  from public.profiles
  where id = p_user_id
    and user_id = auth.uid()
  limit 1;
