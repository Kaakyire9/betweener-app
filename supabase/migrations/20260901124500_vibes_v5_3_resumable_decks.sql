-- Vibes V5.3 resumable decks.
--
-- A brief tab switch, profile detail visit, or app background no longer consumes
-- the user's place. Supabase may pin the latest neutral card for 30 minutes only
-- when that target still survives the authoritative V5 candidate query.
-- Explicit refreshes and decisive actions continue to rotate the deck.
...
declare
  ...
  v_resume_recommendation_id uuid;
begin
  ...
  -- A neutral card remains the user's place in this segment for 30 minutes.
  -- Explicit refreshes and decisive outcomes deliberately bypass continuity.
  if greatest(coalesce(p_refresh_ordinal, 0), 0) = 0 then
    select recommendation.target_profile_id, recommendation.id
      into v_resume_target_id, v_resume_recommendation_id
    from public.vibes_v5_3_recommendations recommendation
    where recommendation.viewer_profile_id = p_user_id
      and recommendation.segment = v_segment
      and recommendation.shown_at >= timezone('utc', now()) - interval '30 minutes'
      and (
        recommendation.outcome is null
        or recommendation.outcome in ('profile_open', 'intro_complete')
      )
    order by recommendation.shown_at desc, recommendation.recommended_at desc
    limit 1;
  end if;
  ...
  selected as materialized (
    select
      ...
      row_number() over (
        order by
          case when interleaved.id = v_resume_target_id then 0 else 1 end,
          interleaved.delivery_slot,
          interleaved.is_exploration,
          interleaved.id
  ...
  request_write as (
    ...
    values (
      ...
      jsonb_strip_nulls(jsonb_build_object(
        'seed_count', (select count(*) from base),
        'resume_recommendation_id', v_resume_recommendation_id,
  ...
  select
    ...
    coalesce(selected.recommendation_reasons, '{}'::jsonb)
      || jsonb_build_object(
        ...
        'resumed', coalesce(selected.id = v_resume_target_id, false),
        'resume_window_minutes', 30
...
comment on function public.get_vibes_recommendations_v5_3(
  ...
  'V5.3 reciprocal recommender with 30-minute per-segment continuity, explicit-refresh rotation, unseen-first delivery, graduated cooldowns, controlled exploration, contextual taste, downstream outcome learning, and privacy-safe attribution.';
