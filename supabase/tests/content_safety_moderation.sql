...
select extensions.is(public.content_safety_assess_private_message('Can you book me a table for Friday?')->>'decision',
  'ALLOW', 'ordinary booking language is not treated as solicitation');
select extensions.is(public.content_safety_assess_private_message('I have a premium gym membership.')->>'decision',
  'ALLOW', 'ordinary membership language is not treated as solicitation');
select extensions.is(public.content_safety_assess_private_message('That was an exclusive interview.')->>'decision',
  'ALLOW', 'ordinary exclusive language is not treated as paid promotion');
select extensions.is(public.content_safety_assess_private_message('I love your private photos from the museum.')->>'decision',
  'ALLOW', 'ordinary private-photo context is not treated as paid promotion');
select extensions.is(
...
select extensions.is(
  (select count(*)::integer from public.rpc_admin_get_content_moderation_events(100)
   where status = 'PENDING_REVIEW'
     and actor_user_id = '97000000-0000-4000-8000-000000000002'::uuid),
