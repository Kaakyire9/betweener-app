...
const shouldLogToConsole = isDev && process.env.NODE_ENV !== "test";
...
export const logger = {
  debug(message: string, ctx?: Ctx) {
    ...
    if (shouldLogToConsole) console.log(message, data ?? "");
  ...
  warn(message: string, ctx?: Ctx) {
    const data = safeCtx(ctx);
    if (shouldLogToConsole) console.warn(message, data ?? "");
    addBreadcrumb(message, data);
  },

  error(message: string, error?: unknown, ctx?: Ctx) {
    const data = safeCtx(ctx);
    if (shouldLogToConsole) console.error(message, data ?? "", error ?? "");
