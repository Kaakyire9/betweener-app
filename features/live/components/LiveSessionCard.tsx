    ...
    <LinearGradient colors={posterUrl ? ['rgba(5,17,15,0.08)', 'rgba(5,17,15,0.94)'] : ['#173933', '#111B1A']} locations={[0.12, 1]} style={styles.card}>
      <View style={styles.topRow}>
        <View style={[styles.status, phase === 'live' && styles.liveStatus]}>
          {phase === 'live' ? <Radio size={13} color="#102522" /> : <CalendarClock size={13} color="#E6CA87" />}
          <Text style={[styles.statusText, phase === 'live' && styles.liveStatusText]}>{phase === 'live' ? 'LIVE NOW' : phase === 'past' ? 'PAST LIVE' : formatLiveCountdown(session.scheduledStart).toUpperCase()}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  status: { minHeight: 30, borderRadius: 15, paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderColor: '#806F4A', backgroundColor: 'rgba(16,34,31,0.86)' }, liveStatus: { backgroundColor: '#E1BE70', borderColor: '#F0D692' }, statusText: { color: '#E8D29B', fontSize: 9, letterSpacing: 1.1, fontFamily: 'Manrope_800ExtraBold' }, liveStatusText: { color: '#102522' },
  ...
  title: { color: '#FFF9EF', fontSize: 29, lineHeight: 34, fontFamily: 'PlayfairDisplay_700Bold', textShadowColor: 'rgba(0,0,0,0.55)', textShadowRadius: 7 }, description: { color: '#D5E0DC', fontSize: 12, lineHeight: 19, marginTop: 7, fontFamily: 'Manrope_500Medium' }, time: { color: '#E7CD91', fontSize: 11, marginTop: 9, fontFamily: 'Manrope_700Bold' },
  footer: { paddingTop: 18, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12 }, meta: { color: '#B7C7C3', fontSize: 10, fontFamily: 'Manrope_600SemiBold' }, metrics: { flexDirection: 'row', gap: 11, alignItems: 'center', flexShrink: 1 }, metric: { color: '#C5D3CF', fontSize: 10, fontFamily: 'Manrope_600SemiBold' }, cta: { color: '#F0D28B', fontSize: 11, fontFamily: 'Manrope_800ExtraBold' },
  saved: { position: 'absolute', right: 18, bottom: 50, width: 24, height: 24, borderRadius: 12, backgroundColor: '#E1BE70', alignItems: 'center', justifyContent: 'center' },
});
